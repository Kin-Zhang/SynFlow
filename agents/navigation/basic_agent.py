        ...
        for target_vehicle in vehicle_list:
            ...
            if (use_bbs or target_wpt.is_junction) and route_polygon:
                ...
                if route_polygon.intersects(target_polygon):
                    ...
                    d_center = target_vehicle.get_location().distance(ego_location)
                    d_bumper = d_center - self._vehicle.bounding_box.extent.x - target_vehicle.bounding_box.extent.x
                    return ObstacleDetectionResult(True, target_vehicle, d_bumper)
            ...
            # Simplified approach, using only the plan waypoints (similar to TM)
                ...
                if is_within_distance(target_rear_transform, ego_front_transform, max_distance, [low_angle_th, up_angle_th]):
                    ...
                    d_center = target_transform.location.distance(ego_transform.location)
                    d_bumper = d_center - self._vehicle.bounding_box.extent.x - target_vehicle.bounding_box.extent.x
                    return ObstacleDetectionResult(True, target_vehicle, d_bumper)
