...
class LocalPlanner:
    ...
    def set_global_plan(self, current_plan, stop_waypoint_creation=True, clean_queue=True):
        ...
        :return:
        """
        # # DEBUG: Track who is setting the global plan
        # import traceback
        # print(f"\n[DEBUG] ===== set_global_plan called =====")
        # print(f"  clean_queue={clean_queue}, new_plan_length={len(current_plan)}")
        # if len(current_plan) > 0:
        #     first_wp = current_plan[0][0].transform.location
        #     last_wp = current_plan[-1][0].transform.location
        #     print(f"  First waypoint: ({first_wp.x:.1f}, {first_wp.y:.1f}, {first_wp.z:.1f})")
        #     print(f"  Last waypoint:  ({last_wp.x:.1f}, {last_wp.y:.1f}, {last_wp.z:.1f})")
        # print(f"  Call stack (last 5):")
    ...
    def run_step(self, debug=False):
        ...
        #     vec_to_target = carla.Vector3D(target_loc.x - ego_loc.x, target_loc.y - ego_loc.y, 0)
        #     dot = vec_to_target.x * ego_fwd.x + vec_to_target.y * ego_fwd.y
        #     dist = ego_loc.distance(target_loc)
            
        #     # Print every 20 frames or when potential U-turn detected
        #     if not hasattr(self, '_debug_frame_cnt'):
        #         self._debug_frame_cnt = 0
        #     self._debug_frame_cnt += 1
            
        #     is_uturn = abs(yaw_diff) > 90 or (dot < 0 and dist > 5)
        #     if self._debug_frame_cnt % 20 == 0 or is_uturn:
        #         print(f"[DEBUG] Frame {self._debug_frame_cnt} | Queue: {len(self._waypoints_queue)}")
        #         print(f"  Ego:    ({ego_loc.x:.1f}, {ego_loc.y:.1f}, {ego_loc.z:.1f}) yaw={ego_yaw:.1f}")
        #         print(f"  Target: ({target_loc.x:.1f}, {target_loc.y:.1f}, {target_loc.z:.1f}) yaw={target_yaw:.1f}")
        #         print(f"  YawDiff: {yaw_diff:.1f}° | Dot: {dot:.1f} | Dist: {dist:.1f}m")
        #         if is_uturn:
        #             print(f"  *** POTENTIAL U-TURN DETECTED! ***")
        # else:
        #     print(f"[DEBUG] Queue EMPTY! Ego: ({ego_loc.x:.1f}, {ego_loc.y:.1f})")
        ...
        # NEW LOGIC: Find the best waypoint to target
        # Strategy: Find the closest waypoint that is AHEAD of the vehicle
        # If all waypoints are behind, find the closest one overall
        
        best_ahead_idx = -1
        best_ahead_dist = float('inf')
        closest_idx = 0
        closest_dist = float('inf')
        
        for i, (waypoint, _) in enumerate(self._waypoints_queue):
            wp_loc = waypoint.transform.location
            dist = ego_loc.distance(wp_loc)
            
            # Vector from ego to waypoint
            vec_to_wp = carla.Vector3D(wp_loc.x - ego_loc.x, wp_loc.y - ego_loc.y, 0)
            dot_product = vec_to_wp.x * ego_fwd.x + vec_to_wp.y * ego_fwd.y
            
            # Track closest waypoint overall
            if dist < closest_dist:
                closest_dist = dist
                closest_idx = i
            
            # Track closest waypoint that is ahead (dot > 0)
            if dot_product > 0 and dist < best_ahead_dist:
                best_ahead_dist = dist
                best_ahead_idx = i
        ...
        if best_ahead_idx >= 0:
            # Found a waypoint ahead - remove all waypoints before it
            # But only if the ahead waypoint is reasonably close (within 15m)
            # or the closest waypoint is behind us
            if best_ahead_dist < 15.0 or best_ahead_idx == closest_idx:
                num_waypoint_removed = best_ahead_idx
        else:
            # All waypoints are behind - this shouldn't happen in normal operation
            # Keep the last waypoint as target (don't remove anything)
            # The vehicle will reach it eventually
            pass
        
        # Additional check: remove waypoints that are very close (passed)
        # This handles the normal case of progressing through waypoints
        while num_waypoint_removed < len(self._waypoints_queue):
            wp, _ = self._waypoints_queue[num_waypoint_removed]
            wp_loc = wp.transform.location
            dist = ego_loc.distance(wp_loc)
            
            # Don't remove the last waypoint unless very close
